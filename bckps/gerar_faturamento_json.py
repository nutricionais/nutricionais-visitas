#!/usr/bin/env python3
"""
gerar_faturamento_json.py
──────────────────────────────────────────────────────────────────────────
Gera o FATURAMENTO_DATA_INLINE (JSON) pronto pra colar no index.html,
a partir de um ou mais arquivos XLSX exportados do Omie.

Uso:
  python3 gerar_faturamento_json.py arquivo1.xlsx [arquivo2.xlsx ...]
  python3 gerar_faturamento_json.py *.xlsx
...
Saída:
  - faturamento_data_inline.json  → JSON puro
  - faturamento_data_inline.js    → trecho JS pronto pra colar no index.html
  - Imprime também um relatório de validação no terminal
...
def main():
    if len(sys.argv) < 2:
        print("Uso: python3 gerar_faturamento_json.py arquivo1.xlsx [arquivo2.xlsx ...]")
        sys.exit(1)
    ...
    print("\n" + "═" * 70)
    print("  GERADOR DE FATURAMENTO_DATA_INLINE — v2")
    print("═" * 70 + "\n")
    ...
    dados = {
        ...
        'cliente_vendedor_mes': gerar_cliente_vendedor_mes(df_vendas),  # ⭐ NOVO Sprint 9.32.31 — cruzamento cliente × vendedor
        'cliente_marca_mes':    gerar_cliente_marca_mes(df_vendas),     # ⭐ NOVO Sprint 9.32.31 — cruzamento cliente × marca
        'cliente_empresa_mes':  gerar_cliente_empresa_mes(df_vendas),   # ⭐ NOVO Sprint 9.32.32 — cruzamento cliente × empresa
        'cliente_produto_mes':  gerar_cliente_produto_mes(df_vendas),   # ⭐ NOVO Sprint 9.32.31 — cruzamento cliente × produto (top 30/cli)
        'produtos_por_cliente': gerar_produtos_por_cliente(df_vendas),
        'marcas':              gerar_marcas(df_vendas),
        'marca_mes':           gerar_marca_mes(df_vendas),  # ⭐ NOVO
    ...
    # Sprint 9.32.30: clientes_lista volta pro JSON principal (inline)
    # Sem fetch separado, sem CORS, funciona em file:// e GitHub Pages igual
    # Sprint 9.32.28: allow_nan=False — falha imediato se sobrar NaN/Infinity
    ...
    try:
        ...
        sys.exit(1)
    print(f"💾 Salvo: {out_json} ({Path(out_json).stat().st_size / 1024:.1f} KB)")

    # Salva trecho JS pronto pra colar
    out_js = 'faturamento_data_inline.js'
    json_str = json.dumps(dados, ensure_ascii=False, separators=(',', ':'), allow_nan=False)
    # Garante que não tenha backtick (senão quebra template literal)
    assert '`' not in json_str, "JSON contém backtick, não dá pra usar como template literal"
    with open(out_js, 'w', encoding='utf-8') as f:
        f.write(f'const FATURAMENTO_DATA_INLINE = `{json_str}`;\n')
    print(f"💾 Salvo: {out_js} ({Path(out_js).stat().st_size / 1024:.1f} KB)")

    print("\n" + "═" * 70)
    print("  PRÓXIMOS PASSOS")
    print("═" * 70)
    print(f"""
  1. Abre '{out_js}' e copia TODA a linha
     (começa com 'const FATURAMENTO_DATA_INLINE = `...`;')

  2. Abre index.html, procura a linha existente (Ctrl+F):
     const FATURAMENTO_DATA_INLINE = `

  3. Substitui a linha inteira pela nova

  4. Salva, commit + push
     git add index.html
     git commit -m "Atualiza faturamento: {dados['meta']['periodo_inicio']} até {dados['meta']['periodo_fim']}"
     git push

  5. Recarrega o dashboard (Ctrl+Shift+R) → dados novos no ar!
""")
